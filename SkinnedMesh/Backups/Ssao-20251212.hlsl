...
float4 PS(VertexOut pin) : SV_Target
{
	...
	for(int i = 0; i < gSampleCount; ++i)
	{
        ...
        if (distZ > -0.1f && dp > 0.01f) // 放宽有效条件
        {
            ...
            float3 rNormal = normalize(gNormalMap.SampleLevel(gsamPointClamp, projQ.xy, 0.0f).xyz);
    ...
    float occlusion = saturate(pow(access, 2.0f)); // 你原来的遮蔽因子
	// Sharpen the contrast of the SSAO map to make the SSAO affect more dramatic.
	
	// 7. 计算最终间接光颜色（乘以强度系数）
    float3 ssgiColor = indirectLightSum; // 从2.0→4.0，甚至6.0
	// 限制颜色范围，避免过亮
    ssgiColor = saturate(ssgiColor + float3(0.05f, 0.05f, 0.05f));
